...
import java.util.LinkedList;
	public class Compiler {
		...
		private static l_var_table Var_l = new l_var_table("",0,"0","0",false,1,0,"");
		private static g_var_table Var_g = new g_var_table("",0,"0","0",false,1,0);
		...
		private static void varseq_st(){//変数宣言の並び
			...
			type = type_st();
				...
				if(i==0){
					l_var_table.add(new l_var_table(tmp1.get(i),type,Var_l.getvar_min(),Var_l.getvar_max(),Var_l.getarray_type(),Var_l.getvar_size(),-1,procedure_name));
					l_counter++;
					offset = offset + Var_l.getvar_size();
				}
				if(i>=1){
					Var_l.setvar_offset(l_var_table.get(l_counter-1).getvar_offset()-l_var_table.get(l_counter-1).getvar_size());
					l_var_table.add(new l_var_table(tmp1.get(i),type,Var_l.getvar_min(),Var_l.getvar_max(),Var_l.getarray_type(),Var_l.getvar_size(),Var_l.getvar_offset(),procedure_name));
					l_counter++;
					offset = offset + Var_l.getvar_size();
				    }
				}
				...
				if(i==0){
					...
					g_counter++;
					g_var_table.add(new g_var_table(tmp1.get(i),type,Var_g.getvar_min(),Var_g.getvar_max(),Var_g.getarray_type(),Var_g.getvar_size(),Var_g.getvar_offset()));
				}
				if(i>=1){
				    Var_g.setvar_offset(g_var_table.get(i-1).getvar_offset()+g_var_table.get(i-1).getvar_size());
				    g_counter++;
				    g_var_table.add(new g_var_table(tmp1.get(i),type,Var_g.getvar_min(),Var_g.getvar_max(),Var_g.getarray_type(),Var_g.getvar_size(),Var_g.getvar_offset()));
				    }
			    }
			...
			if(token.equals("SIDENTIFIER")){
				do{
					...
					if(gl_flag){//グローバル変数
						...
						if(i>=1){
						    Var_g.setvar_offset(g_var_table.get(g_counter-1).getvar_offset()+g_var_table.get(g_counter-1).getvar_size());
						    g_counter++;
						    g_var_table.add(new g_var_table(tmp1.get(i),type,Var_g.getvar_min(),Var_g.getvar_max(),Var_g.getarray_type(),Var_g.getvar_size(),Var_g.getvar_offset()));
						    }
					    }
		...
		private static void instantparaseq_st(){//仮パラメータの並び
			...
			for(int j=0;j<tmp2.size();j++){
				...
				para_counter--;
				proc_list.add(new proc_list(procedure_name,tmp2.get(j),type,Proc.getpara_offset()));
				}
			tmp2.clear();
		}
        ...
        static int variable_st() throws IOException{//変数
        	...
        	boolean flg2 = false;
        	for(int j = 0;j < l_var_table.size();j++){
            	if((l_var_table.get(j).getvar_name().equals(ziku))&&(l_var_table.get(j).getproc_name().equals(procedure_name))){
            	flg1 = true;
            	i_l = j;
            	break;
            	}
        ...
        static int para_st() throws IOException{//パラメーター
        	...
        	for(int j = 0;j < proc_list.size();j++){
            	if(proc_list.get(j).getpara_name().equals(ziku)&&(proc_list.get(j).getproc_name().equals(procedure_name))){
            	i_p = j;
            	flg = true;
            	}
            }
        ...
        private static void input_st() throws IOException{//入力文
        	...
        	if(token.equals("SREADLN")){
                ...
                if(gl_flag){
                	variable_st();
					for(int j = 0;j < g_var_table.size();j++){
						if(g_var_table.get(j).getvar_name().equals(instant_varname1)){
						k = j;
						break;
						}
					}
